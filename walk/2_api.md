...
> **Note**: you should have Node.js installed to use npm command. Go to node.js（https://nodejs.org）web site to learn more details.
...
1. feature file
  create a new feature file under "features" folder, the file name can be "api_testing.feature" or the one you like, edit the content as the following:
  
  ![](assets/2_api_test_feature.png)
...
3. On the feature "api_testing.feature", click each gray button on the right side of each step, to generate the code stubs for them. The code is generated in "definition1.js" file. Notice that the gray button turns orange when there step definition stubs are generated.
...
1. Click the green button ![image](assets/2_green_button.png) behind the step text to open the step definition code that matches the step.
...
2. Validate the mock server is working by navigating to "http://localhost:3000/" in your browser, which should show a web page with message "Congrats! You're successfully running JSON Server."
...
1. Click "Save"![image](assets/2_save_button.png) button to save the report as PDF file.

2. Click "Open File Folder" ![image](assets/2_open_button.png) button to open the folder in explorer where report is located.

3. Click "Show in Browser"![image](assets/2_open_browser_button.png) button to open the report in the default browser.
4. Click "Html Report Theme"![image](assets/2_html_report.png) button to select different theme to render the report.
5. Click "Email"![image](assets/2_email_button.png) button, it will open a new email editing window in Outlook and attach report as an attachment. (Windows only)
...
2. We now update the Scenario Outline and the example table to have the right parameter name and data:
   ...
   b. Change "param2" to "first_name" in both step text and table header. You should also replace "Susan" in DocString to be "\<first_name>". The automatic conversion does not replace the DocString content for you, so you need to do it yourself.
